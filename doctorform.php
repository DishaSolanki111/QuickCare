...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .container { background-color: var(--white); border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.15); width: 100%; max-width: 1400px; padding: 30px 40px; position: relative; }
        ...
        input[type="text"]:focus, input[type="number"]:focus, input[type="email"]:focus, input[type="password"]:focus, input[type="date"]:focus, input[type="file"]:focus, select:focus, textarea:focus { outline: none; border-color: var(--accent-blue); box-shadow: 0 0 0 2px rgba(0,180,216,0.1); }
        ...
        .file-upload-label { display: block; padding: 12px; border: 1px dashed var(--gray-blue); border-radius: 6px; background-color: var(--white); color: var(--soft-blue); text-align: center; transition: all 0.3s; }
        .file-upload-label:hover { border-color: var(--accent-blue); color: var(--primary-blue); background-color: rgba(0,180,216,0.05); }
...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            $field_errors = [
                ...
                'specialisation_id' => '', 'username' => '', 'password' => ''
            ];
            $form_data = [
                'first_name' => '', 'last_name' => '', 'dob' => '', 'doj' => '',
                'gender' => '', 'phone' => '', 'email' => '', 'education' => '',
                'username' => '', 'specialisation_id' => ''
            ];
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $education         = mysqli_real_escape_string($conn, $form_data['education']);
                $username          = $form_data['username'];
                $password          = $_POST['password'] ?? '';
                $specialisation_id = (int)($form_data['specialisation_id']);
                ...
                if (empty($form_data['specialisation_id'])) {
                    $field_errors['specialisation_id'] = "Please select a specialisation.";
                }
                ...
                // ── Final INSERT ──
                // Matches doctor_tbl exactly (DOCTOR_ID=AUTO, STATUS=default 'pending'):
                // SPECIALISATION_ID, PROFILE_IMAGE, FIRST_NAME, LAST_NAME,
                // DOB, DOJ, USERNAME, PSWD, PHONE, EMAIL,
                // GENDER, EDUCATION, SECURITY_QUESTION, SECURITY_ANSWER
                ...
                <div class="form-grid">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <div class="error-message" id="specialisation_id_error"<?php if (!empty($field_errors['specialisation_id'])) echo ' style="display:block"'; ?>><?php echo htmlspecialchars($field_errors['specialisation_id'] ?? ''); ?></div>
                        </div>
    ...
    document.getElementById('dob').addEventListener('input', function () {
        if (this.value && new Date(this.value) <= new Date()) hideError('dob');
        const doj = document.getElementById('doj');
        if (doj.value) doj.dispatchEvent(new Event('input'));
    });
    document.getElementById('doj').addEventListener('input', function () {
        const dojDate = new Date(this.value);
        const dobDate = new Date(document.getElementById('dob').value);
        if (this.value && dojDate <= new Date() && (!document.getElementById('dob').value || dojDate > dobDate)) hideError('doj');
    });
    document.getElementById('specialisation_id').addEventListener('change', function () {
        if (this.value) hideError('specialisation_id');
    });
    ...
    document.getElementById('profile_image').addEventListener('change', function () {
        ...
        if (this.files[0]) {
            ...
            if (!['jpg','jpeg','png'].includes(ext)) {
                ...
                this.value = '';
                document.getElementById('file-label').textContent = 'Choose Profile Image (JPG/PNG)';
            } else {
    ...
    document.getElementById('doctorForm').addEventListener('submit', function (e) {
        ...
        const spec = document.getElementById('specialisation_id').value;
        if (!spec) { showError('specialisation_id', "Please select a specialisation."); isValid = false; }
        else hideError('specialisation_id');
        ...
        if (!isValid) { e.preventDefault(); showToast("Please correct the errors in the form."); }
    });
