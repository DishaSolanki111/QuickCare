 ...
 * Page class
 */
#[AsAlias("RefundTblList", true)]
class RefundTblList extends RefundTbl implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "frefund_tbllist";

    // CSS class/style
    public string $CurrentPageName = "RefundTblList"; // Route action
    ...
    public function __construct(
        ...
        $this->MultiDeleteUrl = "RefundTblDelete";
        $this->MultiUpdateUrl = "RefundTblUpdate";
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->REFUND_ID->AdvancedSearch->toJson(), ","); // Field REFUND_ID
        ...
        $filterList = Concat($filterList, $this->REFUND_AMOUNT->AdvancedSearch->toJson(), ","); // Field REFUND_AMOUNT
        $filterList = Concat($filterList, $this->REFUND_DATE->AdvancedSearch->toJson(), ","); // Field REFUND_DATE
        $filterList = Concat($filterList, $this->REFUND_STATUS->AdvancedSearch->toJson(), ","); // Field REFUND_STATUS
        $filterList = Concat($filterList, $this->REFUND_REASON->AdvancedSearch->toJson(), ","); // Field REFUND_REASON
        $filterList = Concat($filterList, $this->REFUND_TXN_ID->AdvancedSearch->toJson(), ","); // Field REFUND_TXN_ID
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("frefund_tblsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->Command = "search";
        $this->applyFilterToAdvancedSearch("REFUND_ID", $filter); // REFUND_ID
        ...
        $this->applyFilterToAdvancedSearch("REFUND_AMOUNT", $filter); // REFUND_AMOUNT
        $this->applyFilterToAdvancedSearch("REFUND_DATE", $filter); // REFUND_DATE
        $this->applyFilterToAdvancedSearch("REFUND_STATUS", $filter); // REFUND_STATUS
        $this->applyFilterToAdvancedSearch("REFUND_REASON", $filter); // REFUND_REASON
        $this->applyFilterToAdvancedSearch("REFUND_TXN_ID", $filter); // REFUND_TXN_ID
        $this->applyFilterToAdvancedSearch("CREATED_AT", $filter); // CREATED_AT
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = $this->REFUND_REASON;
        $searchFlds[] = $this->REFUND_TXN_ID;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->REFUND_ID); // REFUND_ID
            ...
            $this->updateSort($this->REFUND_AMOUNT); // REFUND_AMOUNT
            $this->updateSort($this->REFUND_DATE); // REFUND_DATE
            $this->updateSort($this->REFUND_STATUS); // REFUND_STATUS
            $this->updateSort($this->REFUND_REASON); // REFUND_REASON
            $this->updateSort($this->REFUND_TXN_ID); // REFUND_TXN_ID
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->REFUND_ID->setSort("");
                $this->PAYMENT_ID->setSort("");
                $this->APPOINTMENT_ID->setSort("");
                $this->PATIENT_ID->setSort("");
                $this->REFUND_AMOUNT->setSort("");
                $this->REFUND_DATE->setSort("");
                $this->REFUND_STATUS->setSort("");
                $this->REFUND_REASON->setSort("");
                $this->REFUND_TXN_ID->setSort("");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if (true) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"refund_tbl\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"refund_tbl\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"refund_tbl\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            if (true) {
                ...
                if ($this->UseAjaxActions && $this->PageID != "grid") {
                    $this->OldKey = $this->getKey(true);
                    $opt->Body = "<a class=\"ew-row-link ew-delete\" data-ew-action=\"inline\" data-action=\"delete\" title=\"" . $deleteTitle . "\" data-caption=\"" . $deleteTitle . "\" data-key= \"" . HtmlEncode($this->getOldKeyAsString()) . "\" data-url=\"" . HtmlEncode(GetUrl($this->DeleteUrl)) . "\">" . $deleteCaption . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"frefund_tbllist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"frefund_tbllist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->REFUND_ID->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"refund_tbl\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "REFUND_ID");
            ...
            $this->createColumnOption($option, "REFUND_AMOUNT");
            $this->createColumnOption($option, "REFUND_DATE");
            $this->createColumnOption($option, "REFUND_STATUS");
            $this->createColumnOption($option, "REFUND_REASON");
            $this->createColumnOption($option, "REFUND_TXN_ID");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frefund_tblsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frefund_tblsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="frefund_tbllist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_refund_tbl", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_refund_tbl",
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frefund_tblsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
