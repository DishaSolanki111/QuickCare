 ...
 * Page class
 */
#[AsAlias("ViewAppointmentReportList", true)]
class ViewAppointmentReportList extends ViewAppointmentReport implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "fview_appointment_reportlist";

    // CSS class/style
    public string $CurrentPageName = "ViewAppointmentReportList"; // Route action
    ...
    public function __construct(
        ...
        $this->TableVar = 'view_appointment_report';
        $this->TableName = 'view_appointment_report';
        ...
        // Initialize URLs
        $this->AddUrl = "ViewAppointmentReportAdd";
        ...
        $this->MultiDeleteUrl = "ViewAppointmentReportDelete";
        $this->MultiUpdateUrl = "ViewAppointmentReportUpdate";
        ...
        if ($url !== null) {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewAppointmentReportView"); // If View page, no primary button
                } else { // List page
    ...
    public function run(): void
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_appointment_reportgrid";
        }
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_appointment_reportsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->applyFilterToAdvancedSearch("STATUS", $filter); // STATUS
    ...
    public function showFilterList(): string
    {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Year, false, false);
        }
        if ($filter != "") {
        ...
        $filter = $this->queryBuilderWhere("APPOINTMENT_TIME");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->APPOINTMENT_TIME, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->APPOINTMENT_TIME->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms(): bool
    {
        ...
        if ($this->APPOINTMENT_TIME->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupSortOrder(): void
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->APPOINTMENT_DATE->Expression . " DESC"; // Set up default sort
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_appointment_reportlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_appointment_reportlist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_appointment_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_appointment_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_appointment_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_appointment_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_appointment_report",
    ...
    protected function loadSearchValues(): bool
    {
        ...
        // APPOINTMENT_TIME
        if ($this->APPOINTMENT_TIME->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->APPOINTMENT_TIME->AdvancedSearch->SearchValue != "" || $this->APPOINTMENT_TIME->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues(?BaseEntity $row = null): void
    {
        ...
        $this->APPOINTMENT_TIME->setDbValue($row['APPOINTMENT_TIME']);
    ...
    protected function newRow(): BaseEntity
    {
        ...
        if (!IsEmpty($this->APPOINTMENT_TIME->DefaultValue)) {
            $row['APPOINTMENT_TIME'] = $this->APPOINTMENT_TIME->DefaultValue instanceof DateTimeInterface ? $this->APPOINTMENT_TIME->DefaultValue : new DateTimeImmutable($this->APPOINTMENT_TIME->DefaultValue);
        }
    ...
    public function renderRow(RowType $rowType = RowType::VIEW, bool $resetAttributes = true): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // APPOINTMENT_TIME
            $this->APPOINTMENT_TIME->setupEditAttributes();
            $this->APPOINTMENT_TIME->EditValue = FormatDateTime(UnFormatDateTime($this->APPOINTMENT_TIME->AdvancedSearch->SearchValue, $this->APPOINTMENT_TIME->formatPattern()), $this->APPOINTMENT_TIME->formatPattern());
            $this->APPOINTMENT_TIME->PlaceHolder = RemoveHtml($this->APPOINTMENT_TIME->caption());

            // STATUS
            if ($this->STATUS->UseFilter && !IsEmpty($this->STATUS->AdvancedSearch->SearchValue)) {
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_appointment_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-table=\"view_appointment_report\" data-caption=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-ew-action=\"modal\" data-url=\"ViewAppointmentReportSearch\" data-btn=\"SearchBtn\">" . $this->language->phrase("AdvancedSearch", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-caption=\"" . $this->language->phrase("AdvancedSearch", true) . "\" href=\"ViewAppointmentReportSearch\">" . $this->language->phrase("AdvancedSearch", false) . "</a>";
        }
        $item->Visible = true;
