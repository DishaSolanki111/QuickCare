 ...
 * Page class
 */
#[AsAlias("ViewFeedbackReportList", true)]
class ViewFeedbackReportList extends ViewFeedbackReport implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "fview_feedback_reportlist";

    // CSS class/style
    public string $CurrentPageName = "ViewFeedbackReportList"; // Route action
    ...
    public function __construct(
        ...
        // Initialize URLs
        $this->AddUrl = "ViewFeedbackReportAdd";
        ...
        $this->MultiDeleteUrl = "ViewFeedbackReportDelete";
        $this->MultiUpdateUrl = "ViewFeedbackReportUpdate";
    ...
    {
        $this->FEEDBACK_ID->setVisibility();
        ...
        if ($url !== null) {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewFeedbackReportView"); // If View page, no primary button
                } else { // List page
    ...
    public function run(): void
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_feedback_reportgrid";
        }
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->FEEDBACK_ID->AdvancedSearch->toJson(), ","); // Field FEEDBACK_ID
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_feedback_reportsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->Command = "search";
        $this->applyFilterToAdvancedSearch("FEEDBACK_ID", $filter); // FEEDBACK_ID
        ...
        $this->applyFilterToAdvancedSearch("RATING", $filter); // RATING
        $this->applyFilterToAdvancedSearch("COMMENTS", $filter); // COMMENTS
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->FEEDBACK_ID); // FEEDBACK_ID
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_feedback_reportlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_feedback_reportlist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "FEEDBACK_ID");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_feedback_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_feedback_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_feedback_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_feedback_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_feedback_report",
    ...
    protected function newRow(): BaseEntity
    {
        ...
        if (!IsEmpty($this->FEEDBACK_ID->DefaultValue)) {
            $row['FEEDBACK_ID'] = intval($this->FEEDBACK_ID->DefaultValue);
        }
        ...
        if (!IsEmpty($this->RATING->DefaultValue)) {
            $row['RATING'] = intval($this->RATING->DefaultValue);
        }
        if (!IsEmpty($this->COMMENTS->DefaultValue)) {
            $row['COMMENTS'] = strval($this->COMMENTS->DefaultValue);
        }
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_feedback_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
