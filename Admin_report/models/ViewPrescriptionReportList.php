 ...
 * Page class
 */
#[AsAlias("ViewPrescriptionReportList", true)]
class ViewPrescriptionReportList extends ViewPrescriptionReport implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "fview_prescription_reportlist";

    // CSS class/style
    public string $CurrentPageName = "ViewPrescriptionReportList"; // Route action
    ...
    public function __construct(
        ...
        // Initialize URLs
        $this->AddUrl = "ViewPrescriptionReportAdd";
        ...
        $this->MultiDeleteUrl = "ViewPrescriptionReportDelete";
        $this->MultiUpdateUrl = "ViewPrescriptionReportUpdate";
    ...
    public function run(): void
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_prescription_reportgrid";
        }
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = Concat($filterList, $this->ISSUE_DATE->AdvancedSearch->toJson(), ","); // Field ISSUE_DATE
        ...
        $filterList = Concat($filterList, $this->SYMPTOMS->AdvancedSearch->toJson(), ","); // Field SYMPTOMS
        ...
        $filterList = Concat($filterList, $this->ADDITIONAL_NOTES->AdvancedSearch->toJson(), ","); // Field ADDITIONAL_NOTES
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_prescription_reportsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->Command = "search";
        $this->applyFilterToAdvancedSearch("PRESCRIPTION_ID", $filter); // PRESCRIPTION_ID
        $this->applyFilterToAdvancedSearch("ISSUE_DATE", $filter); // ISSUE_DATE
        ...
        $this->applyFilterToAdvancedSearch("SYMPTOMS", $filter); // SYMPTOMS
        $this->applyFilterToAdvancedSearch("DIAGNOSIS", $filter); // DIAGNOSIS
        $this->applyFilterToAdvancedSearch("DIABETES", $filter); // DIABETES
        $this->applyFilterToAdvancedSearch("BLOOD_PRESSURE", $filter); // BLOOD_PRESSURE
        $this->applyFilterToAdvancedSearch("ADDITIONAL_NOTES", $filter); // ADDITIONAL_NOTES
    ...
    {
        $where = "";
        $this->buildSearchSql($where, $this->PRESCRIPTION_ID, $default, false); // PRESCRIPTION_ID
        $this->buildSearchSql($where, $this->ISSUE_DATE, $default, false); // ISSUE_DATE
        $this->buildSearchSql($where, $this->Patient_Name, $default, false); // Patient_Name
        $this->buildSearchSql($where, $this->Doctor_Name, $default, false); // Doctor_Name
        $this->buildSearchSql($where, $this->Specialisation, $default, false); // Specialisation
        $this->buildSearchSql($where, $this->SYMPTOMS, $default, false); // SYMPTOMS
        $this->buildSearchSql($where, $this->DIAGNOSIS, $default, false); // DIAGNOSIS
        $this->buildSearchSql($where, $this->DIABETES, $default, false); // DIABETES
        $this->buildSearchSql($where, $this->BLOOD_PRESSURE, $default, false); // BLOOD_PRESSURE
        $this->buildSearchSql($where, $this->ADDITIONAL_NOTES, $default, false); // ADDITIONAL_NOTES
        ...
        if (!$default && $this->Command == "search") {
            $this->PRESCRIPTION_ID->AdvancedSearch->save(); // PRESCRIPTION_ID
            $this->ISSUE_DATE->AdvancedSearch->save(); // ISSUE_DATE
            ...
            $this->SYMPTOMS->AdvancedSearch->save(); // SYMPTOMS
            $this->DIAGNOSIS->AdvancedSearch->save(); // DIAGNOSIS
            $this->DIABETES->AdvancedSearch->save(); // DIABETES
            $this->BLOOD_PRESSURE->AdvancedSearch->save(); // BLOOD_PRESSURE
            $this->ADDITIONAL_NOTES->AdvancedSearch->save(); // ADDITIONAL_NOTES
        }
        return $where;
    }
    ...
    public function showFilterList(): string
    {
        ...
        $filter = $this->queryBuilderWhere("PRESCRIPTION_ID");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->PRESCRIPTION_ID, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->PRESCRIPTION_ID->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field ISSUE_DATE
        $filter = $this->queryBuilderWhere("ISSUE_DATE");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ISSUE_DATE, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ISSUE_DATE->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Doctor_Name, false, false);
        }
        if ($filter != "") {
        ...
        // Field SYMPTOMS
        $filter = $this->queryBuilderWhere("SYMPTOMS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->SYMPTOMS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->SYMPTOMS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field DIAGNOSIS
        $filter = $this->queryBuilderWhere("DIAGNOSIS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->DIAGNOSIS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->DIAGNOSIS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field DIABETES
        $filter = $this->queryBuilderWhere("DIABETES");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->DIABETES, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->DIABETES->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field BLOOD_PRESSURE
        $filter = $this->queryBuilderWhere("BLOOD_PRESSURE");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->BLOOD_PRESSURE, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->BLOOD_PRESSURE->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field ADDITIONAL_NOTES
        $filter = $this->queryBuilderWhere("ADDITIONAL_NOTES");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ADDITIONAL_NOTES, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ADDITIONAL_NOTES->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        $searchFlds[] = $this->SYMPTOMS;
        $searchFlds[] = $this->DIAGNOSIS;
        $searchFlds[] = $this->ADDITIONAL_NOTES;
    ...
    protected function checkSearchParms(): bool
    {
        ...
        if ($this->PRESCRIPTION_ID->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ISSUE_DATE->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->SYMPTOMS->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->DIAGNOSIS->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->DIABETES->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->BLOOD_PRESSURE->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ADDITIONAL_NOTES->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->PRESCRIPTION_ID->AdvancedSearch->unsetSession();
        $this->ISSUE_DATE->AdvancedSearch->unsetSession();
        ...
        $this->SYMPTOMS->AdvancedSearch->unsetSession();
        $this->DIAGNOSIS->AdvancedSearch->unsetSession();
        $this->DIABETES->AdvancedSearch->unsetSession();
        $this->BLOOD_PRESSURE->AdvancedSearch->unsetSession();
        $this->ADDITIONAL_NOTES->AdvancedSearch->unsetSession();
    }
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->ISSUE_DATE); // ISSUE_DATE
            ...
            $this->updateSort($this->SYMPTOMS); // SYMPTOMS
            ...
            $this->updateSort($this->ADDITIONAL_NOTES); // ADDITIONAL_NOTES
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_prescription_reportlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_prescription_reportlist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "ISSUE_DATE");
            ...
            $this->createColumnOption($option, "SYMPTOMS");
            $this->createColumnOption($option, "DIAGNOSIS");
            $this->createColumnOption($option, "DIABETES");
            $this->createColumnOption($option, "BLOOD_PRESSURE");
            $this->createColumnOption($option, "ADDITIONAL_NOTES");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_prescription_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_prescription_reportsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_prescription_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_prescription_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_prescription_report",
    ...
    protected function loadSearchValues(): bool
    {
        ...
        if ($this->PRESCRIPTION_ID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->PRESCRIPTION_ID->AdvancedSearch->SearchValue != "" || $this->PRESCRIPTION_ID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ISSUE_DATE->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ISSUE_DATE->AdvancedSearch->SearchValue != "" || $this->ISSUE_DATE->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->SYMPTOMS->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->SYMPTOMS->AdvancedSearch->SearchValue != "" || $this->SYMPTOMS->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->DIABETES->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->DIABETES->AdvancedSearch->SearchValue != "" || $this->DIABETES->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->BLOOD_PRESSURE->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->BLOOD_PRESSURE->AdvancedSearch->SearchValue != "" || $this->BLOOD_PRESSURE->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ADDITIONAL_NOTES->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ADDITIONAL_NOTES->AdvancedSearch->SearchValue != "" || $this->ADDITIONAL_NOTES->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function newRow(): BaseEntity
    {
        ...
        if (!IsEmpty($this->PRESCRIPTION_ID->DefaultValue)) {
            $row['PRESCRIPTION_ID'] = intval($this->PRESCRIPTION_ID->DefaultValue);
        }
        if (!IsEmpty($this->ISSUE_DATE->DefaultValue)) {
            $row['ISSUE_DATE'] = $this->ISSUE_DATE->DefaultValue instanceof DateTimeInterface ? $this->ISSUE_DATE->DefaultValue : new DateTimeImmutable($this->ISSUE_DATE->DefaultValue);
        }
        ...
        if (!IsEmpty($this->SYMPTOMS->DefaultValue)) {
            $row['SYMPTOMS'] = strval($this->SYMPTOMS->DefaultValue);
        }
        if (!IsEmpty($this->DIAGNOSIS->DefaultValue)) {
            $row['DIAGNOSIS'] = strval($this->DIAGNOSIS->DefaultValue);
        }
        if (!IsEmpty($this->DIABETES->DefaultValue)) {
            $row['DIABETES'] = strval($this->DIABETES->DefaultValue);
        }
        if (!IsEmpty($this->BLOOD_PRESSURE->DefaultValue)) {
            $row['BLOOD_PRESSURE'] = intval($this->BLOOD_PRESSURE->DefaultValue);
        }
        if (!IsEmpty($this->ADDITIONAL_NOTES->DefaultValue)) {
            $row['ADDITIONAL_NOTES'] = strval($this->ADDITIONAL_NOTES->DefaultValue);
        }
        return $row;
    }
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_prescription_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-table=\"view_prescription_report\" data-caption=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-ew-action=\"modal\" data-url=\"ViewPrescriptionReportSearch\" data-btn=\"SearchBtn\">" . $this->language->phrase("AdvancedSearch", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $this->language->phrase("AdvancedSearch", true) . "\" data-caption=\"" . $this->language->phrase("AdvancedSearch", true) . "\" href=\"ViewPrescriptionReportSearch\">" . $this->language->phrase("AdvancedSearch", false) . "</a>";
        }
        $item->Visible = true;
