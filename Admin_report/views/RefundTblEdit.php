...
<?= $Page->getHtmlMessage() ?>
<main class="edit">
<?php $formAction = UrlFor("edit.refund_tbl", $Page->getUrlKey(true)) ?>
<form name="frefund_tbledit" id="frefund_tbledit" class="<?= $Page->FormClassName ?>" action="<?= $formAction ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
ew.on("wrapper", function () {
    ...
    let form = new ew.FormBuilder()
        .setId("frefund_tbledit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["REFUND_ID", [fields.REFUND_ID.visible && fields.REFUND_ID.required ? ew.Validators.required(fields.REFUND_ID.caption) : null], fields.REFUND_ID.isInvalid],
...
<?php if ($Page->REFUND_ID->Visible) { // REFUND_ID ?>
    <div id="r_REFUND_ID"<?= $Page->REFUND_ID->rowAttributes() ?>>
        <label id="elh_refund_tbl_REFUND_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->REFUND_ID->caption() ?><?= $Page->REFUND_ID->Required ? Language()->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->REFUND_ID->cellAttributes() ?>>
<span id="el_refund_tbl_REFUND_ID">
<span<?= $Page->REFUND_ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= $Page->REFUND_ID->getDisplayValue($Page->REFUND_ID->getEditValue()) ?>"></span>
<input type="hidden" data-table="refund_tbl" data-field="x_REFUND_ID" data-hidden="1" name="x_REFUND_ID" id="x_REFUND_ID" value="<?= HtmlEncode(ConvertToString($Page->REFUND_ID->CurrentValue)) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn ew-submit" name="btn-action" id="btn-action" type="submit" form="frefund_tbledit" formaction="<?= $formAction ?>"><?= Language()->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= Language()->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="frefund_tbledit" formaction="<?= $formAction ?>" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= Language()->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
