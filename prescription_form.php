...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_prescription_id'])) {
    ...
    try {
        ...
        $message = "<p class='message success'>Prescription deleted successfully!</p>";
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_prescription'])) {
    try {
        ...
        $stmt->bind_param("isidsssss", $_POST['appointment_id'], $_POST['issue_date'], $height, $weight, $bp, $_POST['diabetes'], $_POST['symptoms'], $_POST['diagnosis'], $_POST['additional_notes']);
        $stmt->execute();
        ...
        $conn->commit();
        $message = "<p class='message success'>Prescription added successfully!</p>";
    } catch (Exception $e) {
        $conn->rollback();
...
// --- FETCH EXISTING PRESCRIPTIONS ---
 $sql = "
    SELECT p.PRESCRIPTION_ID, p.ISSUE_DATE, p.DIAGNOSIS, p.SYMPTOMS,
           a.APPOINTMENT_DATE, d.FIRST_NAME AS DOC_FNAME, d.LAST_NAME AS DOC_LNAME 
...
// --- FETCH PATIENT'S COMPLETED APPOINTMENTS ---
 $appointment_sql = "
    SELECT a.APPOINTMENT_ID, a.APPOINTMENT_DATE, d.FIRST_NAME, d.LAST_NAME, s.SPECIALISATION_NAME
...
<head>
    ...
    <style>
        ...
        .prescription-item {
            ...
            overflow: hidden; /* Contains the floated delete button */
        }
        .prescription-header {
            display: flex;
    ...
    <div class="container">
        ...
        <div class="form-container">
            ...
            <?php if (empty($completed_appointments)): ?>
                <p style="color: #8a6d3b; background-color: #fcf8e3; border: 1px solid #faebcc; padding: 15px; border-radius: 4px;">
                    <strong>Note:</strong> This patient has no completed appointments. A prescription must be linked to a completed visit.
                </p>
            <?php else: ?>
                    ...
                    <div class="form-group">
                        ...
                        <select id="appointment_id" name="appointment_id" required>
                            ...
                            <?php foreach ($completed_appointments as $apt): ?>
                                <option value="<?php echo $apt['APPOINTMENT_ID']; ?>">
                                    <?php echo htmlspecialchars($apt['APPOINTMENT_DATE'] . " with Dr. " . $apt['FIRST_NAME'] . " " . $apt['LAST_NAME'] . " (" . $apt['SPECIALISATION_NAME'] . ")"); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <?php foreach ($prescriptions as $prescription): ?>
                    <div class="prescription-item">
                        <div class="prescription-header">
                            ...
                            <form action="prescription_manager.php?patient_id=<?php echo $patient_id; ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this prescription?');">
